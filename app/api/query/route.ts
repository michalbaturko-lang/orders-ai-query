...
Sloupce pro filtrování: ${columns}
...
Odpovez POUZE validnim JSON.` }]
    ...
    if (queryConfig.aggregation) {
      ...
      if (agg.type === 'count' && !agg.groupBy) {
        ...
        const { data } = await supabase.from(tableName).select(queryConfig.select || '*')
        if (data) {
      ...
      if (queryConfig.filters) {
        for (const f of queryConfig.filters) {
          // Skip raw_data column - it's JSONB and doesn't support text operators
